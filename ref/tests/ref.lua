...
tests.test_operator_chaining_edge_cases = function()
	...
	if mod_ok then
		...
		assert_true(string.find(tostring(mod_val_or_err), "%%0") ~= nil or
			string.find(tostring(mod_val_or_err), "modulo") ~= nil or
			string.find(tostring(mod_val_or_err), "n%%0") ~= nil)
	end
...
if arg and arg[0] and arg[0]:match("ref%.lua$") then
	run_tests()
end
