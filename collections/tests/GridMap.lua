...
if true then
	...
	if not is_lua51 then
		...
		assert(grid:count() == 0, "Grid # operator should return 0 initially")
	end
	-- Test inBounds
	...
	-- Test pairs (on 5.1/LuaJIT __pairs is unsupported, call __pairs directly)
	local pairs_count = 0
	if is_lua51 then
		for i, x, y, value in grid:__pairs() do
