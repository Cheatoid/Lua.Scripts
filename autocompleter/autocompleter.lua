...
local function is_shorthand_match(shorthand, word_to_check)
	if shorthand == "" then return false end -- empty shorthand should not match
...
local function get_completions(self, input_str, options)
	...
	if do_shorthand and input_str ~= "" and #final_completions < max_results then
		for i = 1, #self.all_words do
			if #final_completions >= max_results then break end
	...
	-- 4. Fuzzy matching (sorted by edit distance)
	if do_fuzzy and input_str ~= "" and #final_completions < max_results then
		...
		for i = 1, #self.all_words do
			...
			if distance <= max_edit_distance then
				fuzzy_candidates[#fuzzy_candidates + 1] = { word = self.all_words[i], distance = distance }
			end
		end
		-- Sort fuzzy candidates by distance (closest first)
		table_sort(fuzzy_candidates, function(a, b) return a.distance < b.distance end)
		for i = 1, #fuzzy_candidates do
			if #final_completions < max_results then
				add_completion(fuzzy_candidates[i].word)
			else
				break
			end
		end
...
M.calculate_levenshtein_distance = calculate_levenshtein_distance
M.collect_words = collect_words
M.create_node = create_node
M.is_shorthand_match = is_shorthand_match
