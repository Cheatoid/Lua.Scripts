...
function Console:parse_line(line)
	...
	if not cmd then
		...
		return { raw = line, name = cmdname, cmd = nil, args = {} }
	end
	...
	for i = 1, #specs do
		...
		if raw == nil then
			...
			local val, err = parse_arg_value(spec.type and spec.type or spec, raw)
	...
	if ctx.kind == "ArgValue" then
		...
		if spec and spec.type == "bool" then
			...
			for i = 1, #results do
				local r = results[i]
				out[#out + 1] = {
					key = r.item.key or r.key,
					label = r.item and r.item.key or r.key,
					score = r.score or
						1.0,
					meta = { type = "bool" }
				}
